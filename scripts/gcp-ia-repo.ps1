param(
  ...
  [string]$CommitMessage = "Publish secure GCP-IA project",
  [string]$OwnerEmail
)
...
function Get-DomainUserPrefix {
  ...
  throw "Could not determine the user prefix for repository naming. Set GCP_IA_REPO_USER_PREFIX or authenticate with GitHub first."
}
...
function Resolve-OwnerMetadata {
  ...
  if (-not $email) { $email = $env:GCP_IA_OWNER_EMAIL }
  ...
  if (-not $user -and $env:GCP_IA_REPO_USER_PREFIX) {
    $user = $env:GCP_IA_REPO_USER_PREFIX.ToLowerInvariant()
  }
...
function Invoke-SecurityGate {
  ...
  if ($findings.Count -gt 0) {
    Write-Host "GCP-IA security gate found $($findings.Count) blocker(s):"
  ...
  Write-Host "GCP-IA security gate passed."
}
...
switch ($Action) {
  ...
  "AuthStatus" {
    ...
    [pscustomobject]@{ login = $login; organizations = $orgs; hasGcpIa = ($orgs -contains $Org); organization = $Org } | ConvertTo-Json
    break
  }
