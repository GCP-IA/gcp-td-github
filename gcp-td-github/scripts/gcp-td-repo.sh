...
VISIBILITY="private"
REMOTE="origin"
COMMIT_MESSAGE="Publish secure GCP-TD project"
OWNER_EMAIL=""
...
get_user_prefix() {
  ...
  echo "Could not determine the user prefix. Set GCP_TD_REPO_USER_PREFIX or authenticate with GitHub first." >&2
  exit 1
}
...
resolve_owner_metadata() {
  ...
  local email="${explicit_email:-${GCP_TD_OWNER_EMAIL:-$github_email}}"
  local user=""
...
case "$ACTION" in
  ...
  AuthStatus)
    ...
    login="$(ensure_gcp_td_access "$ORG")"
    gh api user/orgs --jq '{login: "'"$login"'", organizations: [.[] .login], hasGcpTd: any(.[]; .login == "'"$ORG"'")}'
    ;;
