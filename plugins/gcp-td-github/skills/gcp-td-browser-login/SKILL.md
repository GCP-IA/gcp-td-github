...
description: Use when the user needs to connect GitHub for the GCP-TD plugin through GitHub CLI browser authentication without installing tools or requesting administrator rights.
---

# GCP-TD Browser Login
...
Publishing is allowed only when `AuthStatus` confirms access to `GCP-TD`. If the user is authenticated but does not have organization access, stop and ask a GitHub organization admin to add the user or grant the required role.
